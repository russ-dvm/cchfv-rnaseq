...
dds_huh7 <- dds_huh7[ rowSums(counts(dds_huh7)) > 6, ]
...
res_huh7 <- results(dds_huh7, alpha = 0.05, lfcthreshold = 2)


## MA Plot
res1_huh7 <- results(dds_huh7, lfcThreshold=2)
res2_huh7 <- lfcShrink(dds_huh7, contrast=c("status","infected","uninfected"), res=res1_huh7)
plotMA(res2_huh7, ylim = c(-2, 2), main="Huh7")

## Significant only
sig_huh7 <- subset(res_huh7, padj < 0.05)
table(sig_huh7$padj < 0.05)

## Down and upregulated
# downreg <- head(resSig[ order(resSig$log2FoldChange), ])
upreg_huh7 <- head(sig_huh7[ order(sig_huh7$log2FoldChange, decreasing = T), ])


##Heatmap
upreg_top <- head(upreg_huh7, 20)
topVarGenes <- head(order(rowVars(assay(rld_huh7)), decreasing = TRUE), 50)
mat  <- assay(rld_huh7)[ topVarGenes, ]
mat  <- mat - rowMeans(mat)
anno <- as.data.frame(colData(rld_huh7)[, c("timepoint","status")])
pheatmap(mat, annotation_col = anno, main="Huh7")
